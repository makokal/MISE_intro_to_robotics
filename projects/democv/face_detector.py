...
def find_faces_haar(img, face_cascade):
    """ Find all the regions in the image that have faces.
    
    Parameters
    ----------
    img : The input image.
    face_cascade : A pre-trained face detector API.
...
def anonymize_random_colors(img, faces):
...
def anonymize_lines(img, faces, direction='vertical'):
...
if __name__ == '__main__':
    face_cascade = cv.CascadeClassifier('haar_model_frontface.xml')
    ...
    while True:
        ...
        # Detect the faces
        img, faces = find_faces_haar(img, face_cascade)

        # TODO - write this function.
        img = pixelate_faces(img, faces)
        # img = anonymize_random_colors(img, faces)
        # img = anonymize_lines(img, faces, 'both')
